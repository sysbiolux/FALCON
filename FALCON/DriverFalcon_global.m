...
%%% Define your model file and experimental data
    ...
    for f=1:length(ContextsList)
        MeasFileList=[MeasFileList,[ExampleFolder filesep 'ToyDiff_meas_CL' char(ContextsList(f)) 'a.xlsx']];
    end
    Max=10;
%%%
...
%     %%%add noise
%     Noise=1+(rand(size(estim.Output))-0.5).*0.05;
...
figure, imagesc(AllBestx)
set(gca, 'Ytick', 1:length(ListLambda))
set(gca, 'Yticklabels',ListLambda)
set(gca, 'Xtick', 1:length(estim.param_vector))
set(gca, 'Xticklabels',estim.param_vector)
set(gca, 'Xticklabelrotation',45)
title('Regularization plot for ToyDiff Model A')
xlabel('parameter'); ylabel('Lambda')
colormap('hot')

figure, hold on,
X=ListLambda; Y1=AllCosts(:,2); Y2=AllCosts(:,3); Y3=AllCosts(:,1);
plot(X,Y1,'g','linewidth',2), hold on, 
plot(X,Y2,'r','linewidth',1.5), hold on,
plot(X,Y3,'k','linewidth',1), hold on,
legend({'Loss','Reg','Total'})
set(gca, 'XScale','log')
xlabel('Lambda'); ylabel('Costs')

title('Costs for ToyDiff Model A')


%% Re-simulate results based on the best optimised parameter set
[MeanStateValueAll, StdStateValueAll, MeanCostAll, StdCostAll, estim] = FalconSimul_Regularized_L1MSE(estim,bestx,[PlotFitSummary PlotFitIndividual PlotHeatmapCost PlotStateSummary PlotStateEvolution],FinalFolderName);
