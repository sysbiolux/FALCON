function [estim, varargout] = FalconMakeGlobalModel(InputFile, GlobalEdgesList, MeasFileList, ContextsList)
...
estim = FalconMakeModel(InputFile, cell2mat(MeasFileList(1)));
...
for c = 1:size(I, 1) %for each interaction
    ...
    % multiply the interaction by the number of contexts, 
    % unless it is part of the inputs or outputs
    ...
    % multiply all the parameter names
    if str2num(char(thisI(5))) > 0
        % here should we fix the values of the edges?
...
GlobalFile = ['GlobalNetworkFile_' stamp '.txt'];
FalconInt2File(Itot, GlobalFile);

estim = FalconMakeModel(GlobalFile, tempfile);
