...
% ======================================
%%%
% FalconInstall % Uncomment in case the Falcon toolbox has not yet been added to Matlab's path
clc, clear all % clear screen and workspace 

% Define optmisation options
optRound=10; % Number of optimisation rounds
MaxFunEvals=10000; % Number of maximal function being evaluated
MaxIter=10000; % Number of maximal iteration being evaluated
...
PlotBiograph        = 1; % Graph of network topology, nodes activities, and optimised parameters
...
efficency_range = [0:0.2:1]; % indicate the different KO efficencies you want to test (vector from 0 to 1)
% ===================================================
% |||||||||||||||||||||||||||||||||||||||||||||||||||
...
InputFile='NFKB_net.xlsx';
MeasFile='NFKB_data.xlsx';
...
figure, plot(MeanStateValueAll(:,4), MeanStateValueAll(:,6),'.-')
xlabel('IL1'), ylabel('NFKB')
...
if LPSA_Analysis
    ...
    disp(['Estimated Time for LPSA analysis (fast): ' num2str(Estimated_Time_LPSA) ' seconds']); beep; pause(0.5); beep; 
